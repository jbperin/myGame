 ...
 */
public class glDaeTexturedShape extends glRenderableShape {

    private String TAG= "glDaeTexturedShape";

    DaeLoader mShape;

    public glDaeTexturedShape(DaeLoader shape) {
        super ();
        mShape = shape;
        setTextured(true);
    }
    ...
    float[] getVBObuffer(String geomId) {
        FloatBuffer vboBuffer = null;

        short[] indexBuffer = mShape.getMeshFaceIndexBuffer(geomId);

        float[] verts = mShape.getMeshVerticesBuffer(geomId);
        float[] coords = mShape.getMeshTextureCoordinatesBuffer(geomId); // getIBObuffer(TEX_COORDS);
        float[] norms = mShape.getMeshNormalsBuffer(geomId); //getIBObuffer(NORMALS);
    ...
    float[] getVBObuffer() {
        ...
        FloatBuffer vboBuffer = null;
        Collada colladaData = mShape.getTheCollada();

        List<Geometry> lGeom= colladaData.getLibraryGeometries().getGeometries();

        for (Geometry geom: lGeom) {
            Mesh theMesh = geom.getMesh();

            short[] indexBuffer = mShape.getMeshFaceIndexBuffer(geom.getId());

            float[] verts = mShape.getMeshVerticesBuffer(geom.getId());
            float[] coords = mShape.getMeshTextureCoordinatesBuffer(geom.getId()); // getIBObuffer(TEX_COORDS);
            float[] norms = mShape.getMeshNormalsBuffer(geom.getId()); //getIBObuffer(NORMALS);
    ...
    public void render(float[] mMatrixView, float[] mMatrixProjection, InterfaceSceneRenderer Scene) {
        ...
        for (Geometry geom: lGeom) {
            ...
            float [] buffer = getVBObuffer(geom.getId());
            ...
            int textureHandlerId = TextureHandler.getInstance().getTextureId(mShape.getMeshTextureName(geom.getId()));
